                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <div class="d-flex justify-content-between mt-3">
                            <div class="d-flex justify-content-between">
                                <a href="{{ route('petugas.show', $item->petugas_warga_id) }}" class="btn btn-info btn-sm">Detail</a>
                                ...
                                @if(auth()->check() && auth()->user()->role === 'admin')
                                    ...
                                    <form method="POST" action="{{ route('petugas.destroy', $item->petugas_id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus petugas ini?')">Hapus</button>
                                    </form>
                                @endif
                            </div>
                        </div>
